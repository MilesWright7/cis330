...
int main(int argc, char **argv)
{
	...
	// exact same as in non-matrix code
...
void find_nth(int **int_array, uint32_t row, uint32_t col, int n, int *ret_arr)
{
   ...
   // int_array is of the form [i][j] where i is col and j is row.
   // thus 
    if(n > col){
	for(int i = 0; i < col; i++){
	    ret_arr[i] = -1;
	...
	for(int j = 0; j < row - 1; j++){
	    ...
	    for(int k = j + 1; k < row; k++){
	    	if(int_array[k][i] > int_array[max_index][i]){
		    max_index = k;
		}
		else if(int_array[k][i] == int_array[max_index][i]){
			int_array[k][i] = -2;
		}
	    }
	    int temp = int_array[max_index][i];
	    int_array[max_index][i] = int_array[j][i];
	    int_array[j][i] = temp;
	}
    }
    for(int i = 0; i < col; i++){
	ret_arr[i] = int_array[n - 1][i];
    }
   // -------------------------
}
