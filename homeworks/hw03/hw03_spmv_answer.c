              ...
              int *res)
{
	...
	// res already has allocated memory
	// ensure all result values are 0 to start
	for(int k = 0; k < row; k++){
		res[k] = 0;
	}
                        ...
                        int **coo_row, int **coo_col, int **coo_val)
{
    ...
    uint32_t entries = row_ptr[row];
    int *coo_row_ = (int *)malloc(sizeof(int) * entries);
    int *coo_col_ = (int *)malloc(sizeof(int) * entries);
    int *coo_val_ = (int *)malloc(sizeof(int) * entries);
    
	for(int i = 0; i < row; i++) {
        for(int j = row_ptr[i]; j < row_ptr[i + 1]; j++) {
			coo_row_[j] = i;
			coo_col_[j] = col_ind[j];
			coo_val_[j] = val[j];
		}
	}
              ...
              int *coo_val, int *vec, int *res)
{
	...
	// make sure result values are all 0 to start
	for(int k = 0; k < row; k++){
			res[k] = 0;
	}
