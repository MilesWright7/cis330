...
namespace Mem {
  ...
  public:
	...
	friend Mem::IntQueue & operator<<(IntQueue &Q, int input);

	friend Mem::IntQueue & operator>>(IntQueue &Q, int &output);
...
IntQueue &
operator>>(IntQueue &Q, int &output)
{
  int epic = Q.Dequeue();
  output = epic;
  return Q;
